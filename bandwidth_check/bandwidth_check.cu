...
constexpr int NUM_COPY_ITERATIONS = 100;

static void checkBandwidth(cudaStream_t& stream, size_t dataSize) {
    ...
    // Work is issued twice. Once to prime the compute device to init any
    // internal structures. The second time is then used to actually measure
    // the connection bandwidth from host to device.
    for (int i = 0; i < NUM_COPY_ITERATIONS; ++i) {
        ::cudaMemcpyAsync(dData, hData, dataSize * sizeof(float), cudaMemcpyHostToDevice, stream);
    }
    ::cudaStreamSynchronize(stream);
    // Now, we will perform the real timing measurement below.
